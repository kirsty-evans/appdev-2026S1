...
* `bool RemoveMember(Member member)` - Removes a member from the group, you will need to ensure that all members are still accessible and that you have not accidentally cut off someone. If the member does not exist within the `Group`, the method will return `false`, otherwise the method will return `true`.
